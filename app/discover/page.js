...
export default function App() {
  ...
  // load saved (server first, fallback to localStorage)
  useEffect(() => {
    let canceled = false
    async function load() {
      try {
        ...
        if (res.ok) {
          const data = await res.json()
          if (!canceled && Array.isArray(data.items) && data.items.length) {
            setSaved(data.items)
            try { localStorage.setItem('mm_saved', JSON.stringify(data.items)) } catch {}
            return
          }
        }
      } catch {}
  ...
  function saveItem(n) {
    setSaved(prev => {
      ...
      fetch('/api/save', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ neighborhood: n }),
      }).catch(() => {})
      return next
    })
  }
      ...
      <nav className="sticky top-0 z-40 px-4 sm:px-8 py-4 backdrop-blur-xl bg-black/30 border-b border-white/5">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Button onClick={() => setChatOpen(true)} className="rounded-full bg-white text-black hover:bg-white/90 text-sm font-medium gap-2 hidden sm:inline-flex">
